#!/usr/bin/env python3
"""
Microscopic Feature Extraction App
===================================

This application extracts features from a loaded image using a configurable pipeline.
It supports both segmented and non‐segmented images. Two screens are displayed:
an Original Image and a Feature Extraction Preview. The current extraction pipeline order
is shown at the bottom.

File menu (groups separated by horizontal lines):
  • Load Image  
...
All errors are caught and displayed in a dialog (with a “Copy” button if pyperclip is available).
Undo/Redo restores both the base image and the pipeline order.
Mass processing/export uses multiple cores.
"""
